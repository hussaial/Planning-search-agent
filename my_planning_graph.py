...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            ...
            #print(pgNode_a.__dict__)
            if aNode.prenodes.issubset(self.s_levels[level]):
                action_nodes.append(aNode)
        
            for sNode in self.s_levels[level]:
                sNode.children.add(aNode)
                aNode.prenodes.add(sNode)
        
        #self.a_levels[level] = action_nodes
    ...
    def add_literal_level(self, level):
        ...
        for aNode in self.a_levels[level-1]:
            ...
            for s in aNode.effnodes:
                s_literals.add(s)
                #sNode = PgNode_s(s_literals,True)
                #self.s_levels[level].add(PgNode_s(literal, True))
                #call add_action function
                s.parents.add(aNode)
                aNode.children.add(s)
        
        self.s_levels.append(s_literals)
        #self.add_action_level(level)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #a1_effects_pos = node_a1.action.effect_add
        #a2_effects_pos = node_a1.action.effect_add
        #a1_effects_neg = node_a1.action.effect_rem
        #a2_effects_neg = node_a1.action.effect_rem
        #a1_precond_neg = node_a1.action.precond_neg
        #a2-precond_neg = node_a2.action.precond_neg
        #a1_precond_pos = node_a1.action.precond_pos
        #a2-precond_pos = node_a2.action.precond_pos
        
        for a1 in node_a1.action.effect_add:
            if a1 in node_a2.action.precond_neg:
                return True

        for a2 in node_a2.action.effect_add:
            if a2 in node_a1.action.precond_neg:
                return True            
        
        for a1 in node_a1.action.effect_rem:
            if a1 in node_a2.action.precond_pos:
                return True

        for a2 in node_a2.action.effect_rem:
            if a2 in node_a1.action.precond_pos:
                return True   
        return False
...
#==============================================================================
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        #print('state:',node_s1)
        if node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos:#node_s1.__eq__(node_s2.literal):
            return True
        return False
